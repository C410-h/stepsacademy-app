...
const HangmanSVG = ({ errors }: { errors: number }) => (
  <svg viewBox="0 0 120 140" className="w-full max-w-[180px] mx-auto" strokeLinecap="round">
    ...
    {/* 1 — Cabeça */}
    {errors >= 1 && (
      <circle cx="75" cy="35" r="10" stroke="#520A70" strokeWidth="2.5" fill="none" />
    )}
    {/* 2 — Corpo */}
    {errors >= 2 && (
      <line x1="75" y1="45" x2="75" y2="90" stroke="#520A70" strokeWidth="2.5" />
    )}
    {/* 3 — Braço esquerdo */}
    {errors >= 3 && (
      <line x1="75" y1="58" x2="55" y2="75" stroke="#520A70" strokeWidth="2.5" />
    )}
    {/* 4 — Braço direito */}
    {errors >= 4 && (
      <line x1="75" y1="58" x2="95" y2="75" stroke="#520A70" strokeWidth="2.5" />
    )}
    {/* 5 — Perna esquerda */}
    {errors >= 5 && (
      <line x1="75" y1="90" x2="55" y2="115" stroke="#520A70" strokeWidth="2.5" />
    )}
    {/* 6 — Perna direita */}
    {errors >= 6 && (
      <line x1="75" y1="90" x2="95" y2="115" stroke="#520A70" strokeWidth="2.5" />
    )}
  </svg>
);
...
const WordDisplay = ({
  ...
  // Normalize for display — only show letters A-Z (uppercase)
      ...
      {letters.map((char, i) => {
        ...
        return (
          ...
          >
            <span
              className={cn(
                "text-xl font-bold min-w-[1.5rem] text-center",
...
const StepByStep = () => {
  ...
  // ── Load vocab word ───────────────────────────────────────────────────────
  const loadNewWord = async (lid: string | null, sid?: string) => {
    if (!lid) {
      setGameState("no_vocab");
      return;
    }

    // 70% difficulty 1, 30% difficulty 2 or 3
    ...
    let { data: words } = await supabase
      ...
      .eq("level_id", lid)
      .eq("active", true)
      .in("difficulty", diffFilter);

    // Fallback: se não tiver palavras no filtro, busca qualquer
  ...
  const wordLetters = word
    ? new Set(word.word.toUpperCase().split("").filter(c => /[A-Z]/.test(c)))
    : new Set<string>();
  ...
  const handleWin = async () => {
    ...
    await supabase.from("xp_events").insert({
      ...
      xp: xpGain,
      coins: coinsGain,
      description: `Forca correto: ${word.word}`,
    });

    await supabase.from("stepbystep_attempts").insert({
      student_id: sid,
    ...
    if (mission && !mission.completed) {
      ...
      if (nowComplete) {
        ...
        await supabase
          .from("student_gamification")
          .update({
            xp_total: currentXp + xpGain + bonusXp,
            coins: currentCoins + coinsGain + bonusCoins,
            updated_at: new Date().toISOString(),
          })
          .eq("student_id", sid);
      ...
      await supabase.from("daily_missions").update(missionUpdate).eq("id", mission.id);
      setMission(prev =>
        prev
          ? {
              ...prev,
              exercises_done: newDone,
              completed: nowComplete || prev.completed,
  ...
  const handleLoss = async () => {
    ...
    await supabase.from("xp_events").insert({
      ...
      xp: xpGain,
      coins: 0,
      description: `Forca errado: ${word.word}`,
    });
  ...
  // ── Guess handler (defined after handleWin/handleLoss) ───────────────────
  ...
  const handleNextWord = () => {
    loadNewWord(levelId, studentId || undefined);
  };
  ...
  const missionProgress = mission
    ? Math.min((mission.exercises_done / mission.exercises_total) * 100, 100)
    : 0;
  ...
  return (
    <StudentLayout>
      <div className="space-y-4">
            ...
            {missionLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground font-light">
                  {mission?.exercises_done ?? 0}/{mission?.exercises_total ?? 10} exercícios
                </p>
              </>
            )}
          </CardContent>
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">
                {missionDone && !freePlay ? "Prática Livre" : "Mini-game Forca"}
              </CardTitle>
          ...
          <CardContent className="space-y-4">
            {gameState === "no_vocab" ? (
              <div className="py-8 text-center space-y-2">
                ...
                <p className="text-xs text-muted-foreground font-light">
                  Peça ao professor para adicionar vocabulário ao seu nível.
                </p>
              </div>
            ) : (
              <>
                ...
                {gameState === "won" && (
                  <div className="space-y-2">
                    ...
                    {word?.example_sentence && (
                      <p className="text-xs text-center text-muted-foreground italic font-light">
                        "{word.example_sentence}"
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-center">
                      <p className="font-bold text-red-600 text-sm">
                        A palavra era: <span className="uppercase">{word?.word}</span>
                      </p>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-bold">
                    {gamification.streak_current > 0
                      ? `${gamification.streak_current} dias seguidos 🔥`
                      : "Sem streak ativo"}
                  </p>
                  <p className="text-xs text-muted-foreground font-light">
                    {gamification.streak_current > 0
                      ? "Continue amanhã para manter o streak!"
                      : "Faça sua missão hoje para começar um streak."}
                  </p>
                </div>
              </div>
        ...
        {/* ── Prática Livre (após missão concluída) ────────────────────── */}
          ...
          <Card className="border-lime/40 bg-lime/5">
            <CardContent className="py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground font-light">
                  Continue praticando por diversão.
                </p>
              </div>
              <Button
                size="sm"
                className="bg-lime text-steps-black hover:bg-lime/90 font-bold shrink-0"
        ...
        {sessionXp > 0 && (
          <div className="flex items-center justify-center gap-1.5 text-sm text-muted-foreground font-light pb-2">
            ...
            <span>
              <span className="font-bold text-primary">+{sessionXp} XP</span> nesta sessão
            </span>
          </div>
        )}
      </div>
